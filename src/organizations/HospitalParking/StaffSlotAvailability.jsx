...
import './StaffSlotAvailability.css';

const StaffSlotAvailability = () => {
  ...
  // Define Staff parking zones and slots
  const zones = {
    'Staff Zone 1': ['Slot 1', 'Slot 2', 'Slot 3'],
    'Staff Zone 2': ['Slot 1', 'Slot 2', 'Slot 3'],
    'Staff Zone 3': ['Slot 1', 'Slot 2', 'Slot 3'],
  };
  ...
  const handleDownloadTicket = (ticket) => {
    const doc = new jsPDF();
    doc.text(`Staff Parking Ticket`, 10, 10);
    ...
    doc.save(`staff-ticket-${ticket.slot.zone}-${ticket.slot.slot}.pdf`);
  };

  return (
    <div className="staff-slot-availability">
      <h1>Staff Parking Slot Availability</h1>

      {Object.keys(zones).map((zone) => (
        <div key={zone} className="staff-slot-availability__zone">
          <h2>{zone}</h2>
          <div className="staff-slot-availability__slots">
            {zones[zone].map((slot, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`staff-slot-availability__slot ${isBooked ? 'booked' : 'available'}`}
      ...
      {selectedSlot && (
        <div className="staff-slot-availability__booking-form">
      ...
      {ticketDetails.length > 0 && (
        <div className="staff-slot-availability__ticket-display">
          <h3>Generated Tickets</h3>
          {ticketDetails.map((ticket, index) => (
            <div key={index} className="staff-slot-availability__ticket">
...
export default StaffSlotAvailability;
